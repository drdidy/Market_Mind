...
# ═══════════════════════════════════════════════════════════════════════════════════════
# MARKETLENS PRO - PART 3A/3B/3C (Unified)
# Live Data, Asian Session (ES→SPX), Technicals, and Pro Charts — DARK-UI SAFE
# ═══════════════════════════════════════════════════════════════════════════════════════
...
RTH_START = time(8, 30)  # CT
RTH_END   = time(14, 30) # CT

MAJOR_EQUITIES = {
    "^GSPC","AAPL","MSFT","NVDA","AMZN","GOOGL","META","NFLX","TSLA","GOOG","BRK-B","UNH","JNJ","V"
}

CHART_CONFIG = {
    "displaylogo": False,
    "modeBarButtonsToRemove": [
        "lasso2d","select2d","autoScale2d","toggleSpikelines","hoverClosestCartesian","hoverCompareCartesian"
    ],
    "responsive": True
}

# --- Safe helpers for apps that may not define these elsewhere ---
def log_error(msg: str, channel: str = "System"):
    try:
        st.session_state.setdefault("error_log", []).append({"when": datetime.now(), "where": channel, "msg": msg})
    except Exception:
        pass

def get_display_symbol(sym: str) -> str:
    return "SPX" if sym == "^GSPC" else sym

def _get_current_asset() -> str:
    # Try AppState first, then session_state, then fallback
    try:
        from AppState import AppState  # if present
        return AppState.get_current_asset()
    except Exception:
        return st.session_state.get("asset", "^GSPC")

def _get_forecast_date() -> date:
    try:
        from AppState import AppState  # if present
        return AppState.get_forecast_date()
    except Exception:
        return st.session_state.get("forecast_date", date.today())

def get_market_status():
    """Very small helper used by session info (open/closed)."""
    now = datetime.now(ET)
    is_wkday = now.weekday() < 5
    open_t = now.replace(hour=9, minute=30, second=0, microsecond=0)
    close_t = now.replace(hour=16, minute=0, second=0, microsecond=0)
    if is_wkday and open_t <= now <= close_t:
        return "Open", "Regular"
    if is_wkday and now.time() < time(9, 30):
        return "Pre-Market", "Pre"
    if is_wkday and time(16, 0) < now.time() <= time(20, 0):
        return "After Hours", "After"
    return "Closed", "Closed"

# ═══════════════════════════════════════════════════════════════════════════════════════
# 3A. DARK-UI SAFE STYLING + DATA VALIDATION CORE
# ═══════════════════════════════════════════════════════════════════════════════════════

def inject_3x_dark_scope_css():
    """
    Scopes readable text colors to the sections below, even if a previous stylesheet
    forces main text color to black with !important.
    """
    st.markdown("""
    <style>
    /* Scope everything inside .ml-dark-scope to light text */
    .ml-dark-scope, .ml-dark-scope * { color: rgba(229,231,235,0.98) !important; }

    /* Make metrics readable */
    .ml-dark-scope div[data-testid="stMetricLabel"] > div { color: rgba(255,255,255,0.72) !important; }
    .ml-dark-scope div[data-testid="stMetricValue"] { color: #e5e7eb !important; }
    .ml-dark-scope span[data-testid="stMetricDelta"] {
    ...
    /* Alert/readability */
    .ml-dark-scope div[role="alert"] * { color: #e5e7eb !important; }
    .ml-dark-scope div[role="alert"] { background: rgba(255,255,255,0.06) !important; }

    /* DataFrame readability */
    .ml-dark-scope div[data-testid="stDataFrame"] { background: rgba(255,255,255,0.03) !important; }
    .ml-dark-scope div[data-testid="stDataFrame"] * { color: rgba(229,231,235,0.98) !important; }
    </style>
    """, unsafe_allow_html=True)

if "ml_css_scoped" not in st.session_state:
    inject_3x_dark_scope_css()
    st.session_state["ml_css_scoped"] = True
...
def calculate_data_quality_score(data: dict, historical_data: pd.DataFrame = None) -> int:
    ...
    if data.get('status') == 'success': score += 40
    if validate_price_data(data): score += 30
    if data.get('volume', 0) > 0: score += 15
    ts = data.get('timestamp')
    if ts and (datetime.now() - ts).total_seconds() < 300: score += 15
    return min(100, score)
...
def get_real_market_data(symbol: str) -> dict:
    try:
        ...
        tkr = yf.Ticker(symbol)
        info = tkr.info  # can be slow, use carefully
        ...
        if not validate_price_data(result):
            raise ValueError("Price data failed validation")
        return result
    except Exception as e:
        log_error(f"get_real_market_data: {e}", "Data")
        return {
            "symbol": symbol, "price":0.0, "change":0.0, "change_pct":0.0, "volume":0,
...
def get_historical_data(symbol: str, period: str = "1mo", interval: str = "1d") -> pd.DataFrame:
    try:
        if not validate_symbol_format(symbol):
            raise ValueError(f"Invalid symbol: {symbol}")
        tkr = yf.Ticker(symbol)
        hist = tkr.history(period=period, interval=interval, prepost=True)
        if hist is None or hist.empty:
            raise ValueError(f"No historical data for {symbol}")
        hist = hist.reset_index()
        hist["Symbol"] = symbol
        # EMAs if enough data
        if len(hist) >= 21 and "Close" in hist:
        ...
        log_error(f"get_historical_data: {e}", "Data")
        return pd.DataFrame()
...
def get_intraday_data(symbol: str, period: str = "1d", interval: str = "5m") -> pd.DataFrame:
    try:
        ...
        log_error(f"get_intraday_data: {e}", "Data")
        return pd.DataFrame()
...
def get_market_session_info() -> dict:
    ...
    if now_et.weekday() < 5:
        if time(4,0)  <= now_et.time() < time(9,30): session, emoji = "Pre-Market","🌅"
        elif time(9,30) <= now_et.time() < time(16,0): session, emoji = "Regular Hours","📈"
        elif time(16,0) <= now_et.time() < time(20,0): session, emoji = "After Hours","🌆"
        else: session, emoji = "Overnight","🌙"
    else:
        session, emoji = "Weekend","📴"

    if now_et.weekday() < 5:
        open_t  = now_et.replace(hour=9, minute=30, second=0, microsecond=0)
        close_t = now_et.replace(hour=16, minute=0,  second=0, microsecond=0)
        if now_et < open_t: next_event, tte = "Market Open", (open_t - now_et)
        elif now_et < close_t: next_event, tte = "Market Close", (close_t - now_et)
        else:
            next_open = open_t + timedelta(days=1)
            next_event, tte = "Next Market Open", (next_open - now_et)
    else:
        days_until_mon = (7 - now_et.weekday()) % 7 or 1
        next_open = (now_et + timedelta(days=days_until_mon)).replace(hour=9, minute=30, second=0, microsecond=0)
        next_event, tte = "Monday Market Open", (next_open - now_et)

    return {
        "status": basic_status, "session": session, "session_emoji": emoji,
        "timestamp_et": now_et, "timestamp_ct": now_ct,
        "is_trading": session in ["Pre-Market","Regular Hours","After Hours"],
        "next_event": next_event, "time_to_event": tte, "market_day": now_et.weekday() < 5
    }

def get_system_health_metrics() -> dict:
    current_asset = _get_current_asset()
    ...
    api_status = "Connected" if market_data.get("status") == "success" else "Error"
    cache_stats = st.session_state.get("cache_stats", {"hits": 0, "misses": 0})
    total = cache_stats["hits"] + cache_stats["misses"]
    hit_rate = (cache_stats["hits"] / total * 100) if total else 0
    ...
    return {
        ...
        "system_load": min(100, (data_quality + hit_rate) / 2.0)
    }

# ═══════════════════════════════════════════════════════════════════════════════════════
# 3B. ASIAN SESSION (ES→SPX) + ADVANCED PROCESSING — ROBUST IMPLEMENTATION
# ═══════════════════════════════════════════════════════════════════════════════════════

@st.cache_data(ttl=60, show_spinner=False)
def get_es_spx_offset() -> float:
    """Current ES→SPX offset using 1m data; fallback to typical offset."""
    try:
        es = yf.Ticker("ES=F").history(period="1d", interval="1m", prepost=True)
        spx = yf.Ticker("^GSPC").history(period="1d", interval="1m", prepost=True)
        if es is not None and not es.empty and spx is not None and not spx.empty:
            return float(spx["Close"].iloc[-1]) - float(es["Close"].iloc[-1])
    except Exception as e:
        log_error(f"get_es_spx_offset: {e}", "Data")
    return -23.5

def _asian_window_ct(target_d: date) -> tuple[datetime, datetime]:
    prior = target_d - timedelta(days=1)
...
@st.cache_data(ttl=300, show_spinner=False)
def _es_fetch_asian_data(start_ct: datetime, end_ct: datetime, interval: str = "30m") -> pd.DataFrame:
    """
    Robust ES fetch with multiple periods; returns tz-aware CT DataFrame
    ...
    try:
        ...
        for period in ["7d", "1mo"]:
            try:
                tmp = tkr.history(period=period, interval=interval, prepost=True)
        ...
        df = raw.reset_index()
        # Find datetime column produced by yfinance
        dt_col = None
        for c in ["Datetime", "Date", "index"]:
            if c in df.columns:
                dt_col = c; break
        if dt_col is None:  # emergency fallback
            df["Dt"] = pd.to_datetime(df.index)
        else:
        ...
        df = df[req].dropna()
        mask = (df["Dt"] >= (start_ct - timedelta(minutes=60))) & (df["Dt"] <= (end_ct + timedelta(minutes=60)))
        df = df.loc[mask].sort_values("Dt").reset_index(drop=True)
        return df
    except Exception as e:
        log_error(f"_es_fetch_asian_data: {e}", "Data")
        return pd.DataFrame(columns=["Dt","Open","High","Low","Close"])

@st.cache_data(ttl=300, show_spinner=False)
def get_es_asian_session_data(target_date: date, timeframe: str = "30m") -> dict:
    """
    Returns ES Asian session swing CLOSES and SPX equivalents for the window 5–8 PM CT prior day.
    Keys:
      status, es_high_close, es_low_close, spx_high_equivalent, spx_low_equivalent,
      high_time_ct, low_time_ct, asian_range, time_between_extremes, es_spx_offset,
      data_points, timeframe, session_date, method
    """
    try:
        start_ct, end_ct = _asian_window_ct(target_date)
        es = _es_fetch_asian_data(start_ct, end_ct, interval=timeframe)
        if es is None or es.empty:
            return {"status": "error", "message": "No ES data for Asian session"}

        # Use CLOSE-only for line-chart compatibility
        hi_idx = es["Close"].idxmax()
        lo_idx = es["Close"].idxmin()
        es_high_close = float(es.loc[hi_idx, "Close"])
        es_low_close  = float(es.loc[lo_idx, "Close"])
        hi_time = es.loc[hi_idx, "Dt"]
        lo_time = es.loc[lo_idx, "Dt"]

        offset = get_es_spx_offset()
        spx_high_equiv = es_high_close + offset
        spx_low_equiv  = es_low_close  + offset

        asian_range = es_high_close - es_low_close
        tdiff_hours = abs((hi_time - lo_time).total_seconds()) / 3600.0

        # Find the session_date (prior trading day)
        session_date = (start_ct.date())

        return {
            "status": "success",
            "es_high_close": es_high_close,
            "es_low_close":  es_low_close,
            "spx_high_equivalent": spx_high_equiv,
            "spx_low_equivalent":  spx_low_equiv,
            "high_time_ct": hi_time.to_pydatetime(),
            "low_time_ct":  lo_time.to_pydatetime(),
            "asian_range": asian_range,
            "time_between_extremes": tdiff_hours,
            "es_spx_offset": offset,
            "data_points": int(len(es)),
            "timeframe": timeframe.upper(),
            "session_date": session_date,
            "method": "LINE_CHART_CLOSES"
        }
    except Exception as e:
        log_error(f"get_es_asian_session_data: {e}", "Data")
...
def get_previous_day_ohlc(symbol: str, target_date: date) -> dict:
    try:
        ...
        sel = df[df["Date_Only"] == prev]
        if not sel.empty:
            row = sel.iloc[-1]; actual_date = prev
        else:
            row = df.iloc[-1]; actual_date = df["Date_Only"].iloc[-1]
        ohlc = {
        ...
        ohlc["range_pct"] = (ohlc["range"] / ohlc["close"] * 100) if ohlc["close"] else 0.0
        ohlc["body_size"] = abs(ohlc["close"] - ohlc["open"])
        ohlc["upper_wick"] = ohlc["high"] - max(ohlc["open"], ohlc["close"])
        ohlc["lower_wick"] = min(ohlc["open"], ohlc["close"]) - ohlc["low"]
        if ohlc["close"] > ohlc["open"]:
            ohlc["candle_type"], ohlc["candle_color"] = "Bullish","Green"
        elif ohlc["close"] < ohlc["open"]:
            ohlc["candle_type"], ohlc["candle_color"] = "Bearish","Red"
        else:
            ohlc["candle_type"], ohlc["candle_color"] = "Doji","Neutral"
        return ohlc
    except Exception as e:
        log_error(f"get_previous_day_ohlc: {e}", "Data")
        return {"status":"error","message":str(e)}

# Projections (time math helpers)
def calculate_30min_blocks_advanced(from_dt: datetime, to_dt: datetime, exclude_weekends: bool = True) -> int:
    ...
    current = from_dt; blocks = 0
    while current < to_dt:
        if current.weekday() < 5:
            nxt = current + timedelta(minutes=30)
            if nxt <= to_dt: blocks += 1
        current += timedelta(minutes=30)
    return blocks

def generate_rth_projection_slots(base_date: date) -> list[datetime]:
    slots, cur = [], datetime.combine(base_date, RTH_START, tzinfo=CT)
    while cur.time() <= RTH_END:
...
# Simple EMA & momentum analytics (optional use)
def calculate_ema_signals(price_data: pd.DataFrame) -> dict:
    if len(price_data) < 21 or "Close" not in price_data:
        return {"status":"insufficient_data","message":"Need at least 21 periods"}
    try:
        ema8  = price_data["Close"].ewm(span=8,  adjust=False).mean()
        ema21 = price_data["Close"].ewm(span=21, adjust=False).mean()
        cur8, cur21 = ema8.iloc[-1], ema21.iloc[-1]
        cur_px = price_data["Close"].iloc[-1]
        if cur8 > cur21:
            trend = "Bullish"; strength = (cur8 - cur21) / cur21 * 100 if cur21 else 0
        else:
            trend = "Bearish"; strength = (cur21 - cur8) / cur8 * 100 if cur8 else 0
        p8 = (cur_px - cur8) / cur8 * 100 if cur8 else 0
        p21 = (cur_px - cur21) / cur21 * 100 if cur21 else 0
        prev8 = ema8.iloc[-2] if len(ema8) > 1 else cur8
        prev21= ema21.iloc[-2] if len(ema21)> 1 else cur21
        cross = "None"
        if prev8 <= prev21 and cur8 > cur21: cross = "Bullish"
        if prev8 >= prev21 and cur8 < cur21: cross = "Bearish"
        return {
            "status":"success", "current_ema_8":cur8, "current_ema_21":cur21, "current_price":cur_px,
            "trend":trend, "trend_strength":abs(strength), "crossover":cross,
            "price_vs_ema8":p8, "price_vs_ema21":p21, "ema_distance":abs(cur8-cur21), "ema_distance_pct":abs(strength)
        }
    except Exception as e:
        log_error(f"calculate_ema_signals: {e}", "Technical")
        return {"status":"error","message":str(e)}

def analyze_momentum_signals(price_data: pd.DataFrame) -> dict:
    if len(price_data) < 10 or not {"Close","High","Low"}.issubset(price_data.columns):
        return {"status":"insufficient_data"}
    try:
        close = price_data["Close"]
        momentum_5 = close.pct_change(periods=5) * 100
        current_momentum = float(momentum_5.iloc[-1]) if not momentum_5.empty else 0.0
        ...
        if abs(current_momentum) > 2.0: ms = "Strong"
        elif abs(current_momentum) > 0.5: ms = "Moderate"
        else: ms = "Weak"
        md = "Bullish" if current_momentum > 0 else ("Bearish" if current_momentum < 0 else "Neutral")
        return {
            "status":"success","momentum_5":current_momentum,"momentum_direction":md,"momentum_strength":ms,
            "volatility": current_vol, "atr": current_atr,
            "volatility_pct": (current_vol / close.iloc[-1] * 100) if close.iloc[-1] else 0
        }
    except Exception as e:
        log_error(f"analyze_momentum_signals: {e}", "Technical")
        return {"status":"error","message":str(e)}

# ═══════════════════════════════════════════════════════════════════════════════════════
# 3C. PRO CHARTS (Price/EMAs, Volume, Intraday Momentum)
# ═══════════════════════════════════════════════════════════════════════════════════════
...
def create_professional_price_chart(symbol: str, title: str = "Price Chart", show_emas: bool = True):
    ...
    fig.add_trace(go.Scatter(
        x=x_vals, y=df["Close"], mode="lines", name=f"{symbol} Price",
        line=dict(color="#22d3ee", width=3),
    ...
    if show_emas and len(df) >= 21:
        if "EMA_8" in df:
            fig.add_trace(go.Scatter(
                x=x_vals, y=df["EMA_8"], mode="lines", name="8 EMA",
                line=dict(color="#00ff88", width=2, dash="dot"),
                hovertemplate="8 EMA: $%{y:,.2f}<extra></extra>"
            ))
        if "EMA_21" in df:
            fig.add_trace(go.Scatter(
                x=x_vals, y=df["EMA_21"], mode="lines", name="21 EMA",
                line=dict(color="#ff6b35", width=2, dash="dash"),
                hovertemplate="21 EMA: $%{y:,.2f}<extra></extra>"
            ))
    min_p, max_p = float(df["Close"].min()), float(df["Close"].max())
    pr = max_p - min_p; pad = pr * 0.05
    fig.update_layout(
        ...
        yaxis=dict(gridcolor="rgba(255,255,255,0.1)", showgrid=True, zeroline=False, color="#ffffff",
                   range=[max(0, min_p - pad), max_p + pad], tickformat="$,.2f", title="Price"),
        showlegend=True, legend=dict(bgcolor="rgba(0,0,0,0.7)", bordercolor="rgba(255,255,255,0.2)", borderwidth=1),
...
def create_volume_chart_with_data(symbol: str):
    ...
    for i in range(len(df)):
        if i == 0: colors.append("#a855f7")
        else: colors.append("#00ff88" if df["Close"].iloc[i] >= df["Close"].iloc[i-1] else "#ff6b35")
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=x_vals, y=df["Volume"], name="Volume", marker=dict(color=colors, opacity=0.7),
        hovertemplate="<b>%{x}</b><br>Volume: %{y:,}<extra></extra>"
    ))
    if len(df) >= 20:
        df["Volume_MA"] = df["Volume"].rolling(window=20).mean()
        fig.add_trace(go.Scatter(
            x=x_vals, y=df["Volume_MA"], mode="lines", name="Volume MA (20)",
...
def create_intraday_momentum_chart(symbol: str):
    ...
    if df is None or df.empty or "Close" not in df:
        return go.Figure()
    x_vals = df["Datetime"] if "Datetime" in df.columns else df.index
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Price Action","Momentum Indicator"),
                        vertical_spacing=0.15, row_heights=[0.7,0.3])
    fig.add_trace(go.Scatter(
        x=x_vals, y=df["Close"], mode="lines", name="Price",
        line=dict(color="#22d3ee", width=2), hovertemplate="Price: $%{y:,.2f}<extra></extra>"
    ), row=1, col=1)
    if "EMA_8" in df and len(df) >= 21:
        fig.add_trace(go.Scatter(
            x=x_vals, y=df["EMA_8"], mode="lines", name="8 EMA",
            line=dict(color="#00ff88", width=1), hovertemplate="8 EMA: $%{y:,.2f}<extra></extra>"
        ), row=1, col=1)
    if "Momentum" in df:
        mc = ['#00ff88' if x>0 else '#ff6b35' for x in df["Momentum"]]
        fig.add_trace(go.Bar(
            x=x_vals, y=df["Momentum"], name="Momentum", marker=dict(color=mc, opacity=0.7),
...
def get_comprehensive_data_summary() -> dict:
    ...
    sources = {
        "live_market": market_data.get("status") == "success",
        "historical": not historical_data.empty if isinstance(historical_data, pd.DataFrame) else False,
        "asian_session": asian_data.get("status") == "success",
        "previous_day": previous_day.get("status") == "success"
    }
    success = sum(1 for v in sources.values() if v)
    total = len(sources)
    health = (success / total) * 100 if total else 0
    return {
        ...
        "projection_ready": sources["asian_session"] and sources["previous_day"],
        "chart_ready": sources["historical"],
        "live_ready": sources["live_market"]
    }

# ═══════════════════════════════════════════════════════════════════════════════════════
# 3A/3B/3C — UI RENDER (scoped in .ml-dark-scope to fix dark text issue)
# ═══════════════════════════════════════════════════════════════════════════════════════

# Start scoped container so text is readable on dark backgrounds
st.markdown('<div class="ml-dark-scope">', unsafe_allow_html=True)
...
with c3:
    st.metric(session_info["next_event"], f"{session_info['time_to_event'].total_seconds()/3600:.1f}h", "Until Event")

# Health
st.markdown("#### 🔧 System Health Status")
if market_data["status"] == "success":
    st.success(f"✅ Live Data Connected — {display_symbol} via Yahoo Finance")
else:
    st.error(f"❌ Data Connection Error — {market_data.get('error','Unknown error')}")

c1, c2, c3, c4 = st.columns(4)
with c1: st.metric("Data Quality", f"{health_metrics['data_quality']}%", "Score")
with c2: st.metric("API Status", health_metrics["api_status"], "Yahoo Finance")
with c3: st.metric("Cache Hit Rate", f"{health_metrics['cache_hit_rate']:.1f}%", "Performance")
with c4: st.metric("System Uptime", f"{health_metrics['uptime_minutes']:.1f}m", "Session")
...
if asian_data.get("status") == "success":
    st.success(f"✅ Asian Session Data — {asian_data['data_points']} points from {asian_data['session_date']}")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
    ...
    st.error(f"❌ Asian Session Error — {asian_data.get('message','Unknown error')}")

st.markdown("#### 📊 Previous Day OHLC Analysis")
if previous_day_data.get("status") == "success":
    st.success(f"✅ Previous Day Data — {previous_day_data['date']} for {previous_day_data['symbol']}")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Previous High", f"${previous_day_data['high']:,.2f}", previous_day_data["candle_color"])
    with c2: st.metric("Previous Close", f"${previous_day_data['close']:,.2f}", f"Range: {previous_day_data['range_pct']:.2f}%")
    with c3: st.metric("Previous Low", f"${previous_day_data['low']:,.2f}", f"Vol: {previous_day_data['volume']:,}")
    with c4: st.metric("Daily Range", f"${previous_day_data['range']:.2f}", previous_day_data["candle_type"])

    st.markdown("#### 🕯️ Candle Analysis")
    c1, c2, c3 = st.columns(3)
    with c1: st.metric("Body Size", f"${previous_day_data['body_size']:.2f}", "Real Body")
    with c2: st.metric("Upper Wick", f"${previous_day_data['upper_wick']:.2f}", "Top Shadow")
    with c3: st.metric("Lower Wick", f"${previous_day_data['lower_wick']:.2f}", "Bottom Shadow")
else:
    st.error(f"❌ Previous Day Error — {previous_day_data.get('message','Unknown error')}")
...
if isinstance(historical_data, pd.DataFrame) and not historical_data.empty:
    ...
    if ema_signals.get("status") == "success":
        st.success("✅ Technical Analysis Complete — EMA signals calculated")
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.metric("8 EMA", f"${ema_signals['current_ema_8']:,.2f}", f"Trend: {ema_signals['trend']}")
        with c2: st.metric("21 EMA", f"${ema_signals['current_ema_21']:,.2f}", f"Strength: {ema_signals['trend_strength']:.2f}%")
        with c3:
            cross = ema_signals["crossover"]
            icon = "🟢" if cross == "Bullish" else "🔴" if cross == "Bearish" else "🔴"
            label = "None" if cross == "None" else cross
            st.metric("EMA Crossover", f"{icon} {label}", "Signal")
        with c4:
            if momentum_signals.get("status") == "success":
                mdir = momentum_signals["momentum_direction"]
                micon = "🟢" if mdir == "Bullish" else ("🔴" if mdir == "Bearish" else "🟡")
                st.metric("Momentum", f"{micon} {momentum_signals['momentum_strength']}", f"{momentum_signals['momentum_5']:+.2f}%")
            else:
                st.metric("Momentum", "Error", "—")
        c1, c2 = st.columns(2)
        with c1:
            pc8 = ema_signals["price_vs_ema8"]; ic8 = "🟢" if pc8>0 else "🔴"
            st.metric("Price vs 8 EMA", f"{ic8} {pc8:+.2f}%", "Relative Position")
        with c2:
            pc21 = ema_signals["price_vs_ema21"]; ic21 = "🟢" if pc21>0 else "🔴"
            st.metric("Price vs 21 EMA", f"{ic21} {pc21:+.2f}%", "Relative Position")
    else:
...
projection_ready = (asian_data.get("status") == "success" and previous_day_data.get("status") == "success")
if projection_ready:
    st.success("✅ Projection System Ready — Skyline/Baseline calculations possible")
    st.info(f"""
**Available for Projections:**
- Asian Session Anchors (SPX equiv): High {asian_data['spx_high_equivalent']:,.2f} | Low {asian_data['spx_low_equivalent']:,.2f}
- Previous Day Anchors: High {previous_day_data['high']:,.2f} | Low {previous_day_data['low']:,.2f} | Close {previous_day_data['close']:,.2f}
""")
else:
    st.warning("⚠️ Projection System Partial — Some inputs missing")
...
with tab1:
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    colbig, colctl = st.columns([3,1])
    with colctl:
        show_emas = st.checkbox("Show EMAs", value=True)
    fig_price = create_professional_price_chart(current_asset, f"{display_symbol} Price Analysis", show_emas)
    st.plotly_chart(fig_price, use_container_width=True, config=CHART_CONFIG)
    st.markdown('</div>', unsafe_allow_html=True)

with tab2:
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    fig_vol = create_volume_chart_with_data(current_asset)
    st.plotly_chart(fig_vol, use_container_width=True, config=CHART_CONFIG)
    st.markdown('</div>', unsafe_allow_html=True)

with tab3:
    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
    fig_mom = create_intraday_momentum_chart(current_asset)
...
c1, c2, c3, c4 = st.columns(4)
with c1:
    color = "🟢" if system_summary["system_health"] >= 75 else ("🟡" if system_summary["system_health"] >= 50 else "🔴")
    st.metric("System Health", f"{color} {system_summary['system_health']:.0f}%", f"{system_summary['successful_sources']}/{system_summary['total_sources']} Sources")
with c2:
    dq = system_summary["market_data_quality"]
    color = "🟢" if dq >= 75 else ("🟡" if dq >= 50 else "🔴")
    st.metric("Data Quality", f"{color} {dq}%", "Market Data")
with c3:
    st.metric("Projection System", "🟢 Ready" if system_summary["projection_ready"] else "🔴 Incomplete", "Anchors Available")
with c4:
    st.metric("Chart System", "🟢 Active" if system_summary["chart_ready"] else "🔴 Unavailable", "Historical Data")

st.markdown("#### 📡 Data Source Status")
ds = system_summary["data_sources"]
c1, c2, c3, c4 = st.columns(4)
with c1: st.metric("Live Market", "🟢 Connected" if ds["live_market"] else "🔴 Error", "Yahoo Finance")
with c2: st.metric("Historical Data", "🟢 Available" if ds["historical"] else "🔴 Missing", "Price History")
with c3: st.metric("Asian Session", "🟢 Retrieved" if ds["asian_session"] else "🔴 Failed", "ES Futures")
with c4: st.metric("Previous Day", "🟢 Retrieved" if ds["previous_day"] else "🔴 Failed", "OHLC Data")
...
st.info(f"""
**Data System Summary:**
- Live Market Data: {'✅' if ds['live_market'] else '❌'} Real-time (Yahoo Finance)
- Historical Analysis: {'✅' if ds['historical'] else '❌'} 8/21 EMAs
- Asian Session: {'✅' if ds['asian_session'] else '❌'} ES (5–8 PM CT) → SPX
- Previous Day: {'✅' if ds['previous_day'] else '❌'} OHLC Anchors
- Chart Integration: {'✅' if system_summary['chart_ready'] else '❌'} Plotly
- Projection Ready: {'✅' if system_summary['projection_ready'] else '❌'} Inputs present
""")

st.markdown("#### ⚡ Performance Metrics")
perf = get_system_health_metrics()
c1, c2, c3 = st.columns(3)
with c1: st.metric("Cache Hit Rate", f"{perf['cache_hit_rate']:.1f}%", "Data Caching")
with c2: st.metric("Session Uptime", f"{perf['uptime_minutes']:.1f}m", "Current Session")
with c3: st.metric("Error Count", f"{perf['error_count']}", "Session Errors")
