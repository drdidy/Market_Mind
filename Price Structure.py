...
with col2:
    projected_date = st.date_input("Projected Date", value=datetime.today())
    projected_time = st.time_input("Projected Time", value=(datetime.now() + timedelta(minutes=30)).time())
    projected_datetime = datetime.combine(projected_date, projected_time)
...
st.caption("This simplified model uses custom CDF for Black-Scholes. Results may vary with real-time order flow, gamma, and skew.")
